...
- **Signer nodes** — each signer independently watches both chains, witnesses deposits, signs withdrawals, and broadcasts fully-signed Bitcoin transactions. No signer coordinates with another — correctness is enforced by the contracts.
...
3. The signer checks `is_deposit_witnessed(txid, vout, amount, destination_address)` on the bridge contract. If not yet witnessed, it calls `witness_deposit(txid, vout, amount, destination_address)`.
...
4. The signer checks `is_withdraw_signed(btc_transaction)` on the registry contract. If not yet signed, it signs the PSBT and calls `sign_withdraw(btc_transaction, btc_signatures)`.
